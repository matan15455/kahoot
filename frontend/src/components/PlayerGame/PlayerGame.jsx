...
export default function PlayerGame() {
          ...
          {Object.entries(room.summary.answersCount).map(([answer, count]) => (
            <li
              ...
              className={`summary-item
                ${room.summary.correctAnswer === answer ? "correct-answer" : ""}
                ${selectedAnswer === answer && room.summary.correctAnswer !== answer ? "wrong-answer" : ""}
              `}
            >
  ...
  if (room.phase === "SCORES") {
    return (
      <div className="player-game-container">
        <ScoreTable players={room.players} />
        <p className="waiting-text">⏳ ממתינים למארח להמשך המשחק…</p>
      </div>
    );
  }
